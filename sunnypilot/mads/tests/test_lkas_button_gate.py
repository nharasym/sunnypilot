...
try:  # on-device / CI the compiled native modules exist; locally, stub them before importing mads
  ...
  pyx_stub = _stub_native("openpilot.common.params_pyx")
  pyx_stub.Params = type("Params", (), {"get": lambda *a, **k: None, "get_bool": lambda *a, **k: False})
  pyx_stub.UnknownKeyName = type("UnknownKeyName", (Exception,), {})
...
class TestAllowAlwaysBrands:
  @staticmethod
  def _init_mads(brand, monkeypatch):
    monkeypatch.setattr(mads_module, "StateMachine", lambda s: SimpleNamespace(state=0))
    monkeypatch.setattr(mads_module, "read_steering_mode_param", lambda *a: 0)
  ...
  def test_toyota_is_allow_always(self, monkeypatch):
    assert self._init_mads("toyota", monkeypatch).allow_always is True

  def test_other_brand_stays_gated(self, monkeypatch):
    assert self._init_mads("honda", monkeypatch).allow_always is False
