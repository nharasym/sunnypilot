...
#   python -m unittest openpilot.sunnypilot.device_lock.tests.test_device_lock -v
...
class FakeParams:
  ...
  Signatures MUST match common/params.py exactly. A previous version accepted an
...
class TestPinHashing(unittest.TestCase):
  ...
  def test_hash_pin_roundtrip(self):
    stored = hash_pin("0123")
    assert check_pin("0123", stored)
    assert not check_pin("0132", stored)

  def test_hash_pin_salted_differently_each_time(self):
    """Same PIN must not produce the same stored value twice (random salt)."""
    assert hash_pin("0123") != hash_pin("0123")

  def test_plaintext_pin_never_in_stored_value(self):
    assert "0123" not in hash_pin("0123")

  def test_check_pin_rejects_malformed_stored(self):
    for bad in ("", "nosalt", "zz$zz", None):
      assert not check_pin("0123", bad)

  def test_validate_pin_accepts_valid(self):
    for pin in ["0123", "01230123"]:
      with self.subTest(pin=pin):
        validate_pin(pin)

  def test_validate_pin_rejects_invalid(self):
    for pin in ["012", "012301230", "abcd", "01a3", "", "4567", "0129"]:
      with self.subTest(pin=pin):
...
class TestLockUnlock(unittest.TestCase):
  ...
  def test_lock_forces_offroad_mode(self):
    lock = make_lock()
    lock.set_pin("0123")
    lock.lock()
  ...
  def test_unlock_with_wrong_pin_stays_locked(self):
    lock = make_lock()
    lock.set_pin("0123")
    lock.lock()
  ...
  def test_unlock_restores_previous_offroad_mode(self):
  ...
  def test_unlock_clears_offroad_mode_if_it_was_off(self):
    lock = make_lock()
...
class TestEnforce(unittest.TestCase):
  ...
  def test_enforce_reverts_local_offroad_toggle_off(self):
  ...
  def test_enforce_is_noop_when_unlocked(self):
    lock = make_lock()
    lock._params.put_bool(PARAM_OFFROAD_MODE, False)
    lock.enforce()
  ...
  def test_clearing_locked_param_is_the_only_way_out(self):
...
class TestRateLimiting(unittest.TestCase):
  ...
  def test_correct_pin_refused_while_rate_limited(self):
    lock = make_lock()
    lock.set_pin("0123")
    lock.lock()
    for _ in range(RATE_LIMIT_AFTER_ATTEMPTS):
  ...
  def test_successful_unlock_resets_attempts(self):
    lock = make_lock()
    lock.set_pin("0123")
    lock.lock()
    lock.try_unlock("1111")
  ...
  def test_attempts_persist_in_params(self):
    lock = make_lock()
    lock.set_pin("0123")
    lock.lock()
    lock.try_unlock("1111")
  ...
  def test_no_pin_set_cannot_unlock(self):
...
class TestOnroadSafety(unittest.TestCase):

  def test_lock_refused_while_onroad(self):
  ...
  def test_lock_allowed_when_offroad(self):
    lock = make_lock()
    lock.set_pin("0123")
  ...
  def test_lock_refused_if_car_goes_onroad_mid_dialog(self):
  ...
  def test_enforce_does_not_drop_control_on_remote_lock_mid_drive(self):
  ...
  def test_enforce_applies_once_parked(self):
  ...
  def test_unlock_is_allowed_onroad(self):
...
class TestPinFormatMigration(unittest.TestCase):

  def test_pin_written_records_current_format(self):
    lock = make_lock()
    lock.set_pin("0123")
    assert lock._params.get(PARAM_PIN_FORMAT) == PIN_FORMAT_CURRENT

  def test_stale_format_pin_is_treated_as_absent(self):
  ...
  def test_setting_a_new_pin_clears_the_stale_format(self):
    lock = make_lock()
...
class TestParamsApiStrictness(unittest.TestCase):

  def test_no_params_call_uses_a_kwarg_the_real_api_lacks(self):
    ...
    the lock screen. Parse the real signatures out of params.py and check our call sites.
    """
    import pathlib
    import re

    root = pathlib.Path(__file__).parents[3]
    params_src = (root / "common" / "params.py").read_text()

    allowed: dict[str, set[str]] = {}
    for m in re.finditer(r"^  def (get|get_bool|put|put_bool|remove)\(self,([^)]*)\)", params_src, re.M):
      ...
      allowed[name] = kwargs
    assert allowed, "could not parse Params signatures from params.py"
...
class TestAlertRegistry(unittest.TestCase):
  ...
  def test_alert_registry_keys_are_declared_params(self):
    """Regression: Offroad_DeviceLocked was registered in alerts_offroad.json but never declared in
    params_keys.h. Params raises UnknownKeyName on undeclared keys, so the mici alerts-refresh
  ...
  def test_lock_alert_key_exists_in_registry(self):
...
class TestRemoteUnlockPath(unittest.TestCase):

  def test_lock_params_are_remotely_writable(self):
